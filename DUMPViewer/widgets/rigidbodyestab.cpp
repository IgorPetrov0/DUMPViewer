    ...
    switch(ui->meshSourceComboBox->currentIndex()){
        case(0):{//файл
            ...
            if(!name.isEmpty()){
                ...
                body=new rigidBody(&gObject,MESH_ORIGINAL);
            }
            break;
        }
        case(1):{//mainMesh
            body=new rigidBody(app->currentObject()->getMainMesh(),MESH_MAIN_MESH);
            break;
        }
        case(2):{//LOD1
            body=new rigidBody(app->currentObject()->getLod(0),MESH_LOD1);
            body->setSource(MESH_LOD1);
            break;
        }
        case(3):{//LOD2
            body=new rigidBody((graphicObject*)app->currentObject()->getLod(1),MESH_LOD2);
            body->setSource(MESH_LOD2);
            break;
        }
        case(4):{//LOD3
            body=new rigidBody((graphicObject*)app->currentObject()->getLod(2),MESH_LOD3);
            body->setSource(MESH_LOD3);
            break;
        }
        case(5):{//LOD4
            body=new rigidBody(app->currentObject()->getLod(3),MESH_LOD4);
...
void rigidBodyesTab::bodyChangeSlot(){
    ...
    if(item!=NULL){
        currentBody=app->currentObject()->getPhisycObject()->getRigitBody(item->data(Qt::UserRole).toInt());
        if(currentBody!=NULL){
            disconnectAll();
