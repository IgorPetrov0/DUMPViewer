...
var RLANG = {
	...
	horizontalrule: 'Убрать текст под CUT',
	deleted: 'Deleted',
	anchor: 'Anchor',
	...
	Redactor.prototype = {
		...
		build: function(mobile, whendone)
		{
			if (mobile !== true)
			{
				...
				if (this.$editor)
				{
					this.$editor.addClass('redactor_editor').attr('contenteditable', true).attr('dir', this.opts.direction).attr('id', 'redactor_editor_doom_id');
				}
		...
		buildButton: function(key, s)
		{
                    if (key=="horizontalrule"){
                        var button = $('<a href="javascript:void(null);" title="' + s.title + '" class="redactor_btn_' + key + '" style="font-size:10px;line-height:10px;background:transparent;text-indent:0;width:75px;text-align:center;padding-top:3px;height:22px;">'+s.title+'</a>');
                    }else{
